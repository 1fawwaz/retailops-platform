...
\*\*Backend scope (updated 2026-08-14).\*\* StockPilot Core's live surface (`contracts/stockpilot-api/versions/v1.json`, 66 paths / 88 operations, verified directly against the OpenAPI export) now covers Auth (register/login/refresh/logout, password reset, `/me`), Products, Categories, Brands, Suppliers, Warehouses, Inventory (stock, low-stock, dead-stock, slow-movers, valuation, transfers, adjustments, ledger), Purchase Orders, Customers, Sales Orders, Analytics (revenue, profit, turnover, ABC, top/bottom products, period comparison, supplier rollup, PO KPIs), Forecast (demand + accuracy), Notifications, Audit Logs, and Administration (Users, Roles, Settings). No remaining "confirmed missing" module from the 2026-07-31 gap analysis is still absent; the only flagged open dependency is the password-reset delivery channel (admin/support-mediated, not emailed — see `api/routers/auth.py`). `docs/stockpilot-gaps.md` documents the original analysis; see `BUILD.md`'s Backend Module Order for the full target surface.
